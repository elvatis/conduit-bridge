> Note (2026-09-08, codex/agent-orchestrator-dashboard): Added the full-page Elvatis execution workspace, shared Effort/Faster speed controls, public Codex execution events, persistent chat projects, repository analytics and Git/worktree views. Git navigation remains visible above analytics. Scoped APIs enforce ownership, read/admin permissions and repository policy. Windows validation: 611 tests/70 files, 31 targeted checks after the final history-loading correction, build, Secret Scan and whitespace check pass. Chat/Git/analytics passed 390/1920/3840px browser QA. The local build runs on 31338 with seven retained chats; BitNet is healthy on 8080. MIT PR #119 merged separately after required checks, and GitHub About was updated. Feature work remains on the local branch; no feature push or release. See docs/validation/execution-workspace.md and the Codex/Claude Code gap analyses.
