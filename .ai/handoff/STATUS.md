> Focused UX/UI and CLI review and remediation (2026-09-10, PR #120): Conducted a focused review across the entire Web UI and CLI, resolving all truncation, navigation ambiguity, and interactivity gaps. (1) Truncated and Unreadable Text: Removed rigid white-space clipping and arbitrary slicing across the web dashboard and CLI output. Cockpit runs table and workspace paths now wrap cleanly (`word-break: break-all; overflow-wrap: anywhere;`) with full tooltip titles; system tools catalog binary paths wrap without ellipsis cutoffs; execution cards and git workspace elements (`gw-filter-branch`, `gw-branch`, `gw-head`, `gw-author`, `gw-commit-subject`, `exCommandCard`, `ex-workspace-context`, `ex-row-action`) now include full descriptive titles; execution settings grid gained responsive media queries for screens below 850px and 550px; `conduit-bridge runs list`, `runs get`, and `sessions list` now display complete model names, prompts, step outputs, and session titles without arbitrary slicing. (2) Missing Navigation Context: Implemented accessible, hierarchical appbar breadcrumbs (`#app-breadcrumbs`) reflecting current section, subview, and active entity with click-to-navigate support; enforced strict hidden display rules and WCAG 2.5.8 minimum target sizing on breadcrumb buttons; wired real-time breadcrumb synchronization across section transitions and chat sessions (`pfTab`, `pfOpenSession`, `pfPrepareNewChat`); added auto-expansion for collapsed sidebar details when navigating to nested sections; added a contextual quick-navigation cue bar in the Cockpit for instant access to Webchat, Execution, Git, Analytics, and Insights. (3) CLI Discoverability & Interactivity: Created interactive onboarding menu (`src/cli-menu.ts`, accessible via `conduit-bridge menu` or launching with no arguments in an interactive terminal) with server health, CLI provider detection, and numbered quick actions; added live slash command auto-completion (`Tab`) in terminal TUI chat; added dynamic slash command descriptions in the prompt footer bar while typing `/`; added fuzzy "did you mean?" suggestions on misspelled subcommands and slash commands; added a responsive terminal progress spinner during task execution polling; updated TUI help view with `/rollback` and `/retry`. (4) Validation: 22/22 Playwright browser tests passed across all 5 viewport breakpoints in English and German; 70/70 targeted Vitest tests passed; clean production build; zero em-dash or en-dash characters added.

> Automated and operator workspace rollback for agent runs (2026-09-10, PR #120): Resolved the architectural open item where half-written workspace state was left behind after a failed or cancelled agent run. (1) Git Workspace Rollback: Added `action: 'rollback'` to `GitWorkspaceService` performing atomic tree reset via `git reset --hard HEAD` and `git clean -fd -e .conduit-worktrees`, safely restoring modified files while protecting child worktree directories and permitting rollback on detached HEADs. (2) Platform Runs Service: Extended `PlatformRunInput` with `rollbackOnFailure?: boolean` (and CLI `--rollback-on-failure`), automatically triggering workspace rollback if an agent execution fails or is cancelled. Added operator-driven rollback action via `POST /v1/platform/runs/:id/rollback` or `/actions` (`{ action: 'rollback' }`), guarded to disallow rollback on running tasks and requiring a configured `workingDirectory`. (3) TUI & CLI Integration: Added `/rollback [runId]` slash command in interactive chat, single-key `[B]` rollback shortcut in runs list and run-detail views with updated header/footer action hints, and scriptable `conduit-bridge runs rollback <runId>` CLI command. (4) Validation: Verified with targeted test suites in `test/git-workspace.test.ts`, `test/platform-runs.test.ts`, `test/interactive-cli.test.ts`, and `test/cli-commands.test.ts`; clean production build and AAHP governance check.

> Structured execution event telemetry and CI stabilization (2026-09-10, PR #120): (1) CI Stabilization: Synchronized AAHP manifest checksums and resolved Playwright budget form submit timing race in `src/dashboard.ts` by setting `ui_limits_saved` only after `refresh()` settles and confirming save notification before testing invalid input in `test/browser/dashboard.pw.ts`. (2) Structured Execution Telemetry: Connected `ExecutionEvent` pipeline from server to client over SSE (`choices: [], executionEvent: event`) across `/v1/chat/completions` and `/v1/platform/sessions/:id/messages`. Updated `parseSseChunk` in `src/interactive-cli.ts` to extract typed `ExecutionEvent` payloads and wired `onEvent` callback in `client.send` and TUI inference loop to update `state.currentTool` and `state.status` directly from structured commands (`kind: 'command'`), replacing fragile text regex scraping while preserving delta fallback. (3) Background Polling Optimization: Added light polling mode to `refreshExtras` in `src/interactive-cli.ts` to only query `listRuns` during the 2500ms idle timer, cutting background polling HTTP requests by 80%. Validation: 22/22 Playwright browser tests pass, targeted Vitest suites pass, production build clean, aahp check and verify clean.

> Strict agent confinement default with explicit opt-in (2026-09-10, PR #120): Resolved the owner policy decision on unconfined agent providers. Unconfined providers (cli-claude, cli-gemini, cli-grok) that bypass/skip native permissions are now blocked by default in agent mode (HTTP 403 permission_denied) across /v1/chat/completions, /v1/platform/runs, and pipeline step execution. Confined providers (cli-codex with --sandbox workspace-write) remain permitted by default. Explicit opt-in via --allow-unconfined CLI flag, allowUnconfined: true in request payload, or allowUnconfined in provider agent policy permits unconfined providers. Chat and plan modes remain unblocked. Tested and proved with agent-confinement, cli-mode, and server test suites. Validation: npm run build clean, aahp check clean.

> Note (2026-09-10, claude-opus-5): TUI repair series, 13 commits on 4e6eb96..2bcb736, CI green on each. Fixed: control bytes from model output reaching the frame (new src/tui-sanitize.ts); parseSseChunk not understanding this server own chunk format, which made the whole reported blind flight - no delta ever arrived; non-ASCII input silently discarded (36 of 44 code units of a German sentence); terminal not restored on signals or timer throws; createRun fabricating a run id on failure; column width counted in UTF-16 code units so the diff addressed inside wide glyphs (new src/tui-width.ts, a cell is now a column); the suite reading the developer real runtime directory; missing Synchronized Output and bracketed paste; frame cost proportional to the whole session (26.1 ms at 1500 messages); tool and diff cards unreachable at runtime; no polling so a pending approval went unnoticed; POSIX kill reaching only the direct child. NOT PROVED, stated deliberately: the process group change has no test (verify on POSIX by cancelling a provider that spawned a child), two poller guards are reasoned rather than mutation-proved, and the continuation-cell skip in cellsToAnsi stays green under mutation. OPEN and needing the owner decision: agent mode runs three of four providers with the permission check disabled (src/cli-mode.ts:412, :418, :438), which contradicts the stated requirement that nothing be created outside the opened folder; /status now names them but the policy is unchanged. Also open: no rollback after a failed run, no headless vault fallback, tool detection greps model text instead of using the structured event channel that already exists. Full findings with file and line are in the PR 120 comment.

> Responsive TUI layout rewrite (2026-09-10, PR #120): `src/tui-layout.ts` now owns ANSI-aware word wrap at spaces and hyphens, style carry across lines, middle-truncation for unsplittable paths/IDs, breakpoint profiles (large >=120 sidebar 25-28, medium 80-119 sidebar 20, small <80 single column with a Tab strip, tiny <60x15 overlay), and a chat scroll slice with stick-to-bottom. `conduit-bridge tui`/`chat` listens on `process.stdout` resize, resets the differential buffer, and fully reflows; PgUp/PgDn and wheel keep a scrolled viewport clamped inside the box. Low height (<25 rows) collapses the header to one line and the footer to Ctrl+K/Ctrl+P/Ctrl+Q. Mutation cut of wrapAnsi (hard slice) turned all six wrap tests red, then restore green. Validation: `npm run build` clean; Vitest 78 files / 791 tests. No new branch or PR.

> Boxed TUI, silent chat daemon and markup contract (2026-09-10, PR #120): Strengthened `test/dashboard-markup.test.ts` to scan dashboard plus all UI modules for `$('id')` / `getElementById('id')` against `DASHBOARD_HTML`. Obsolete `summary-requests` / `summary-active` remain absent; i18n helpers keep null guards. Chat/TUI now boxes the header, session and prompt with `╭─╮` frames, uses a cell-level differential renderer with no `\x1b[2J` on keystrokes, and starts a missing listener as a detached `start` process logging to `.conduit/logs/bridge.log`. Help is boxed (`USAGE:`, `CORE COMMANDS:`, providers with PATH ready/missing). Validation: `npm run build` clean; Vitest 77 files / 769 tests; Playwright 57/57. No new branch or PR.

> CLI & TUI Enterprise Overhaul and CI Stabilization (2026-09-10, PR #120): Delivered complete 3-phase systems, CLI/TUI and full-stack architecture implementation on `codex/agent-orchestrator-dashboard`. (1) Phase 1 Bugfix & CI Stabilization: Hardened `src/ui/i18n.ts` with null-guards, removed 4 obsolete summary DOM calls in `src/dashboard.ts`, added markup regression test `test/dashboard-markup.test.ts`, verified 57/57 Playwright UI tests and all Vitest suites. (2) Phase 2 Logging & High-End Help Screen: Installed `warning-filter.ts` suppressing experimental Node SQLite warnings, suppressed `.env` log dumps, instituted strict zero-noise interactive mode routing logs to `.conduit/logs/bridge.log` with console muting, and implemented ANSI help screen (`conduit-bridge help`) with 2-column command layout, live PATH availability verification for CLI binaries (`cli-gemini`, `cli-claude`, `cli-codex`, `cli-grok`), API backends, local engines, and workflow examples including `--approval=<mode>`. (3) Phase 3 High-Interaction Terminal Agent TUI Architecture: Implemented `TuiDifferentialRenderer` with virtual double buffering and delta line diffing (100% jitter- and flicker-free typing without full-screen redraws), 60 FPS throttled frame scheduler, dynamic ANSI progressbars (`renderProgressBar`) for codebase indexing, multi-step task runner (`[████████░░░░░░░░] 50%`) and header context window consumption with green/yellow/red color thresholds, pre-execution prompt token estimation (`estimateTokens`) with projected context fill ratio, live token streaming rate tracking (`tokensPerSec`), post-execution turn metrics breakdown (`inputTokens`, `outputTokens`, `totalTokens`, `tokensPerSec`, `turnCostUsd`, `costBudgetPercent`), interactive Human-In-The-Loop approval modal card with syntax-highlighted diff preview (`[y] Approve`, `[e] Edit/Feedback`, `[s] Skip step`, `[c] Abort`), prompt history navigation (`Up`/`Down`), and word-boundary jumping (`findPrevWord`, `findNextWord`, `Ctrl+Left`/`Ctrl+Right`). Validation: 76/76 test files, 764/764 Vitest tests green, 57/57 Playwright UI browser tests green, `npm run build` 100% clean, AAHP lint clean.
