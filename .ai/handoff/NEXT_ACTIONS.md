> Current review point: The requested selection/category correction is implemented. Local checks pass 690 tests, nine affected browser/accessibility cases and all 21 synthetic native checks. BitNet ranks IDs bound to supported original statements; conservative English/German gates omit ambiguous or unsupported phrasing. The current seven test/help chats correctly produce no selected insights from 34 messages, and that report survives restart. Review PR #120 and docs/validation/session-insights.md, including precision/recall limits. Recheck all hosted checks on the new head; merge/release remain outside this task. Older plans below are historical.
