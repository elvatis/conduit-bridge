...
export const PLATFORM_SCRIPT = String.raw`
  ...
  function pfApplyRole() {
    ...
    for (const id of ['pf-new-chat','pf-save-chat','pf-delete-chat','pf-inspect-context','pf-summary-save','pf-run-start','pf-new-memory','pf-memory-save','pf-evaluation-start','pf-vault-save','pf-vault-scan']) $(id).disabled = !operate || pfState.busy;
  ...
  async function pfNewSession() {
    ...
    const result = await pfApi('/sessions', { title: $('pf-chat-title').value.trim() || t('ph_new_conversation'), model: $('pf-chat-model').value, retention: $('pf-chat-retention').value || 'retained', agentId: $('pf-chat-agent').value || undefined, profileId: $('pf-chat-profile').value || undefined, workspaceId });
