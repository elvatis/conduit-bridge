import { effortControl, EFFORT_SCRIPT } from './ui/effort-slider.js';
...
export const PLATFORM_HISTORY_HTML = `<div class="sidebar-history">
<details id="pf-history" open><summary><span data-i18n="ui_chat_history">Chat history</span><span id="pf-history-count" class="pf-count">0</span></summary>
<label><span class="visually-hidden" data-i18n="lbl_find_conversation">Find a conversation</span><input id="pf-session-search" type="search" placeholder="Search chats and projects" data-i18n-ph="ph_search_chat_projects"></label>
<button type="button" id="pf-new-project" class="pf-project-new"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg><span class="action-label" data-i18n="btn_new_chat_project">New project</span></button>
<form id="pf-project-form" class="pf-project-form" hidden><label><span data-i18n="lbl_project_name">Project name</span><input id="pf-project-name" required maxlength="100"></label><div><button type="submit" id="pf-project-save"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg><span class="action-label" data-i18n="btn_save">Save</span></button><button type="button" id="pf-project-cancel"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="m6 6 12 12M6 18 18 6"/></svg><span class="action-label" data-i18n="btn_cancel">Cancel</span></button></div><button type="button" id="pf-project-delete" hidden><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="m6 6 12 12M6 18 18 6"/></svg><span class="action-label" data-i18n="btn_delete">Delete</span></button></form>
<div id="pf-project-status" role="status" class="pf-project-status"></div><div id="pf-sessions" class="platform-list"></div></details></div>`;
...
export const PLATFORM_STYLE = String.raw`
  ...
  .platform-message header small { font-size: 0.8rem; color: var(--muted); overflow-wrap: anywhere; }
  .platform-message-content { white-space: pre-wrap; overflow-wrap: anywhere; line-height: 1.65; font-size: 1rem; }
  .platform-message-actions { display: flex; flex-wrap: wrap; gap: 6px; margin-top: 12px; }
  .platform-message-actions button { padding: 3px 8px; font-size: 0.8rem; }
  ...
  .platform-context-summary { font-size: 0.8571rem; color: var(--muted); overflow-wrap: anywhere; margin-bottom: 8px; }
  .platform-status { min-height: 24px; color: var(--muted); font-size: 0.9286rem; margin: 8px 0; overflow-wrap: anywhere; }
  .platform-status.error { color: var(--bad); }
  .platform-badge { display: inline-block; font-size: 0.8rem; padding: 2px 7px; border-radius: 5px; background: var(--panel-3); color: var(--blue-soft); }
  ...
  .platform-stat strong { display: block; color: var(--text); font-size: 1.4286rem; }
  .platform-stat span { color: var(--muted); font-size: 0.8571rem; }
...
export const PLATFORM_HTML = decorateSettingTooltips(String.raw`
    <div id="platform-section" class="wide page-section platform-shell active">
      ...
      <div class="platform-tabs" role="tablist" aria-label="Workspace views" data-i18n-aria="ui_workspace_views">
        ...
        <button type="button" role="tab" id="pf-tab-vault" aria-selected="false" aria-controls="pf-pane-vault" data-pf-tab="vault" data-i18n="tab_vault">Vault</button>
      ...
      <section id="pf-pane-chat" class="platform-pane" role="tabpanel" aria-labelledby="pf-tab-chat">
        <div class="platform-split">
          ...
          <div class="platform-panel chat-surface">
            <div class="pf-chat-organize"><strong id="pf-active-chat-title" data-i18n="ui_new_chat">New chat</strong><label><span data-i18n="lbl_chat_project">Project</span><select id="pf-chat-project"><option value="" data-i18n="ui_no_chat_project">No project</option></select></label></div>
            ...
            <details class="platform-context"><summary data-i18n="ui_model_context_retention">Model, context &amp; retention</summary>
              <div class="platform-fields">
                ...
                <label><span data-i18n="lbl_retention">Retention</span><select id="pf-chat-retention" disabled><option value="retained" data-i18n="retention_retained_description">Retained: save on this device</option></select></label>
              ...
              <small data-i18n="ui_retention_description">Changing models preserves the conversation transcript. Only the selected memories and skill versions are attached. All conversations are saved on this device until you delete them. Failed and interrupted requests remain in the vault.</small>
            ...
            <form id="pf-chat-form" class="platform-composer"><label><span data-i18n="lbl_your_message">Your message</span><textarea id="pf-chat-input" maxlength="100000" placeholder="Ask a question or describe a task. Ctrl+Enter sends." data-i18n-ph="ph_chat_message" required></textarea></label><div class="platform-toolbar composer-bottom"><label class="composer-model"><span data-i18n="lbl_next_reply_model">Model for the next reply</span><select id="pf-chat-model"></select></label>${effortControl('pf-chat-effort', '#pf-chat-model')}<button id="pf-chat-send" type="submit" class="primary"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polygon points="6 3 20 12 6 21 6 3"/></svg><span class="action-label" data-i18n="btn_send_message">Send message</span></button><button id="pf-chat-stop" type="button" class="danger" disabled><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="15" x2="9" y1="9" y2="15"/><line x1="9" x2="15" y1="9" y2="15"/></svg><span class="action-label" data-i18n="btn_stop_reply">Stop reply</span></button><span id="pf-chat-progress" class="muted" role="status"></span></div></form><p class="chat-footnote" data-i18n="ui_chat_footnote">Your AI, your choice. Check important answers.</p>
          </div>
        </div>
      </section>
      <section id="pf-pane-vault" class="platform-pane" role="tabpanel" aria-labelledby="pf-tab-vault" hidden>
        <div class="platform-panel"><h3 data-i18n="h_vault">Message vault</h3><p class="muted" data-i18n="ui_vault_description">All conversations stay on this device. Search their full text and turn recurring patterns into better prompts.</p>
          <form id="pf-vault-search-form" class="platform-toolbar"><label><span data-i18n="lbl_vault_query">Search every message</span><input id="pf-vault-query" type="search" maxlength="500" required></label><label><span data-i18n="lbl_vault_mode">Search mode</span><select id="pf-vault-mode"><option value="text" data-i18n="ui_vault_text">Full text · SQLite</option><option value="regex" data-i18n="ui_vault_regex">Regular expression · tgrep</option></select></label><button type="submit" data-i18n="btn_vault_search">Search vault</button></form>
          <p class="muted" data-i18n="ui_vault_search_help">Text mode finds words and phrases. Regex mode uses temporary local text files, removed after each search; ripgrep is the fallback when tgrep is unavailable.</p>
          <p id="pf-vault-result-status" class="platform-status" role="status"></p><div id="pf-vault-results" class="platform-artifacts"></div>
        </div>
        <div class="platform-panel" style="margin-top:16px"><h3 data-i18n="h_vault_suggestions">Prompt improvements</h3><p class="muted" data-i18n="ui_vault_scan_help">BitNet scans excerpts from your own messages in recurring batches while the bridge is running. Suggestions include sources and can be used as drafts.</p>
          <form id="pf-vault-settings-form" class="platform-toolbar"><label><span data-i18n="lbl_vault_enabled">Recurring local scans</span><select id="pf-vault-enabled"><option value="true" data-i18n="status_enabled">Enabled</option><option value="false" data-i18n="status_disabled">Disabled</option></select></label><label><span data-i18n="lbl_vault_interval">Interval in minutes</span><input id="pf-vault-interval" type="number" min="5" max="10080" value="60" required></label><button id="pf-vault-save" type="submit" data-i18n="btn_save_details">Save details</button><button id="pf-vault-scan" type="button" data-i18n="btn_vault_scan">Scan now</button></form>
          <p id="pf-vault-status" class="platform-status" role="status"></p><div id="pf-vault-suggestions" class="platform-artifacts"></div>
        </div>
      </section>
      ...
      <section id="pf-pane-library" class="platform-pane" role="tabpanel" aria-labelledby="pf-tab-library" hidden>
        <details class="platform-panel" style="margin-bottom:16px" id="pf-presets-panel"><summary data-i18n="h_coding_presets">Coding pipeline presets</summary><p class="muted" data-i18n="ui_coding_presets_description">Install an evidence-focused workflow with approval checkpoints. Choose a default model, then override individual roles if needed.</p><label><span data-i18n="lbl_preset_default_model">Default model for preset roles</span><select id="pf-preset-model"></select></label>${effortControl('pf-preset-effort', '#pf-preset-model')}<details class="platform-context"><summary data-i18n="ui_role_overrides">Role model overrides</summary><div class="platform-fields"><label><span data-i18n="role_planner">Planner</span><select id="pf-preset-planner"></select></label>${effortControl('pf-preset-planner-effort', '#pf-preset-planner', '', '', '#pf-preset-model')}<label><span data-i18n="role_implementer">Implementer</span><select id="pf-preset-implementer"></select></label>${effortControl('pf-preset-implementer-effort', '#pf-preset-implementer', '', '', '#pf-preset-model')}<label><span data-i18n="role_reviewer">Reviewer</span><select id="pf-preset-reviewer"></select></label>${effortControl('pf-preset-reviewer-effort', '#pf-preset-reviewer', '', '', '#pf-preset-model')}<label><span data-i18n="role_security_reviewer">Security reviewer</span><select id="pf-preset-security"></select></label>${effortControl('pf-preset-security-effort', '#pf-preset-security', '', '', '#pf-preset-model')}</div></details><p id="pf-preset-role-note" class="muted"></p><div id="pf-presets-list" class="platform-artifacts"></div><p id="pf-preset-status" class="platform-status" role="status"></p><button type="button" id="pf-preset-open" hidden><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m9 18 6-6-6-6"/></svg><span class="action-label" data-i18n="btn_open_installed_pipeline">Open installed pipeline</span></button></details>
...
export const PLATFORM_SCRIPT = EFFORT_SCRIPT + String.raw`
  ...
  function pfApplyRole() {
    ...
    setLocalizedText($('pf-operator'), () => pfOperatorName(operator) + ' · ' + localizedValue(operator.role) + ' · ' + ((operator.workspaceIds || []).includes('*') ? t('ui_all_authorized_workspaces') : (operator.workspaceIds || []).length + (' ' + t('ui_authorized_workspaces'))));
    $('pf-tab-system').hidden = !admin;
    for (const option of $('pf-view-select').options || []) if (option.value === 'system') option.disabled = !admin;
    for (const id of ['pf-new-project','pf-project-save','pf-project-delete','pf-chat-project','pf-new-chat','pf-save-chat','pf-delete-chat','pf-inspect-context','pf-summary-save','pf-run-start','pf-new-memory','pf-memory-save','pf-evaluation-start','pf-vault-save','pf-vault-scan']) $(id).disabled = !operate || pfState.busy;
    $('pf-chat-send').disabled = !operate || pfState.busy;
    $('pf-vault-scan').disabled = !operate || pfState.vault?.settings?.status === 'running';
  ...
  async function platformRefresh() {
    ...
    pfState.refreshPromise = (async () => {
      ...
      jobs.push(Promise.all([pfApi('/sessions'), pfApi('/projects')]).then(([sessions, projects]) => { pfState.sessions = pfList(sessions); pfState.projects = pfList(projects); pfRenderSessions(); }));
      if (pfState.tab === 'chat') {
      ...
      } else if (pfState.tab === 'vault') {
        jobs.push(pfApi('/vault').then(pfRenderVault));
      ...
      platformSyncModels();
      if (typeof repositoryWorkspaceSync === 'function') repositoryWorkspaceSync();
      pfApplyRole();
  ...
  function pfRenderSessions() {
    const query = $('pf-session-search').value.trim().toLowerCase();
    const owner = pfState.session?.userId || pfState.operator?.operatorId;
    pfOptions('pf-chat-project', pfState.projects.filter(project => !owner || project.userId === owner), () => t('ui_no_chat_project'), project => project.id, project => project.name);
    if (pfState.session) $('pf-chat-project').value = pfState.session.projectId || '';
    setLocalizedText($('pf-active-chat-title'), () => pfState.session?.title || t('ui_new_chat'));
    $('pf-history-count').textContent = String(pfState.sessions.length);
    const row = session => '<button type="button" data-pf-session="' + esc(session.id) + '" class="' + (session.id === pfState.session?.id ? 'active' : '') + '" title="' + esc((session.title || '') + ' · ' + pfDate(session.updatedAt)) + '"' + (session.id === pfState.session?.id ? ' aria-current="true"' : '') + '><strong>' + esc(session.title || t('ui_untitled_conversation')) + '</strong><small>' + esc(pfDate(session.updatedAt)) + '</small></button>';
    const matches = session => String(session.title || '').toLowerCase().includes(query);
    setLocalizedHtml($('pf-sessions'), () => {
      const groups = pfState.projects.map(project => {
        const all = pfState.sessions.filter(session => session.projectId === project.id);
        const sessions = project.name.toLowerCase().includes(query) ? all : all.filter(matches);
        if (query && !sessions.length && !project.name.toLowerCase().includes(query)) return '';
        const open = query || pfState.projectOpen.get(project.id) === true || (pfState.projectOpen.get(project.id) !== false && pfState.session?.projectId === project.id);
        return '<details class="pf-project-group" data-pf-project="' + esc(project.id) + '"' + (open ? ' open' : '') + '><summary><span>' + esc(project.name) + '</span><small class="pf-count">' + all.length + '</small></summary><div class="pf-project-actions"><button type="button" data-pf-project-new="' + esc(project.id) + '" title="' + esc(t('ui_new_chat')) + '"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg><span class="action-label">' + esc(t('ui_new_chat')) + '</span></button><button type="button" data-pf-project-edit="' + esc(project.id) + '" aria-label="' + esc(t('btn_rename_project')) + '" title="' + esc(t('btn_rename_project')) + '"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="m16 3 5 5-12 12-6 1 1-6ZM14 5l5 5"/></svg></button></div>' + (sessions.map(row).join('') || '<p class="platform-empty">' + esc(t('ui_no_conversations')) + '</p>') + '</details>';
      }).join('');
      const ungrouped = pfState.sessions.filter(session => !session.projectId || !pfState.projects.some(project => project.id === session.projectId)).filter(matches);
      return groups + (ungrouped.length ? '<div class="pf-ungrouped"><span class="pf-group-label">' + esc(t('ui_no_chat_project')) + '</span>' + ungrouped.map(row).join('') + '</div>' : '') || '<p class="platform-empty">' + esc(t('ui_no_conversations')) + '</p>';
    });
    const operate = ['admin','operator'].includes(pfState.operator?.role) && !pfState.busy;
    $('pf-sessions').querySelectorAll?.('[data-pf-project-new],[data-pf-project-edit]').forEach(button => { button.disabled = !operate; });
  }
  function pfEditProject(id) {
    const project = pfState.projects.find(item => item.id === id);
    pfState.editingProject = project || null;
    $('pf-project-name').value = project?.name || '';
    $('pf-project-form').hidden = false;
    $('pf-project-delete').hidden = !project;
    $('pf-project-name').focus();
  }
  async function pfSaveProject() {
    const previous = pfState.editingProject;
    const result = await pfApi('/projects' + (previous ? '/' + encodeURIComponent(previous.id) : ''), { name: $('pf-project-name').value.trim(), expectedRevision: previous?.revision }, previous ? 'PATCH' : 'POST');
    const project = pfEntity(result, 'project');
    pfState.projectOpen.set(project.id, true);
    $('pf-project-form').hidden = true;
    await platformRefresh();
  }
  async function pfMoveChat() {
    const selected = $('pf-chat-project').value || null;
    if (!pfState.session) return;
    const id = pfState.session.id, epoch = pfState.sessionEpoch;
    const result = await pfApi('/sessions/' + encodeURIComponent(id), { projectId: selected, expectedRevision: pfState.session.revision }, 'PATCH');
    if (epoch === pfState.sessionEpoch && pfState.session?.id === id) pfState.session = pfEntity(result, 'session');
    if (selected) pfState.projectOpen.set(selected, true);
    await platformRefresh();
  }
  async function pfOpenSession(id) {
    ...
    if (pfState.session) {
      ...
      pfState.attachments.set(pfState.session.id, { skills: pfValues('pf-chat-skills'), memories: pfValues('pf-chat-memories'), profileId: $('pf-chat-profile').value, agentId: $('pf-chat-agent').value, effort: $('pf-chat-effort').value, fastMode: effortFastMode('pf-chat-effort') ?? false });
    }
    ...
    $('pf-chat-retention').value = pfState.session.retention || 'retained';
    ...
    $('pf-chat-effort').value = attachments?.effort || '';
    setEffortFastMode('pf-chat-effort',attachments?.fastMode ?? false);
  ...
  async function pfNewSession() {
    const selectedAttachments = { skills: pfValues('pf-chat-skills'), memories: pfValues('pf-chat-memories'), profileId: $('pf-chat-profile').value, agentId: $('pf-chat-agent').value, effort: $('pf-chat-effort').value, fastMode: effortFastMode('pf-chat-effort') ?? false };
    const workspaceId = $('pf-chat-workspace').value || undefined;
    const result = await pfApi('/sessions', { title: $('pf-chat-title').value.trim() || t('ph_new_conversation'), model: $('pf-chat-model').value, retention: $('pf-chat-retention').value || 'retained', agentId: $('pf-chat-agent').value || undefined, profileId: $('pf-chat-profile').value || undefined, projectId: $('pf-chat-project').value || undefined, workspaceId });
  ...
  async function pfSendChat() {
    ...
    try {
      ...
      accepted = !(response.headers.get('Content-Type') || '').includes('text/event-stream');
      ...
      if ((response.headers.get('Content-Type') || '').includes('text/event-stream')) {
        await pfReadEventStream(response, event => {
          if (event.type === 'saved' || event.type === 'done') accepted = true;
  ...
  async function pfSearchVault() {
    const query = $('pf-vault-query').value.trim();
    const result = await pfApi('/vault/search?query=' + encodeURIComponent(query) + '&mode=' + encodeURIComponent($('pf-vault-mode').value || 'text'));
    setLocalizedText($('pf-vault-result-status'), () => t('ui_vault_results', { total: result.total, messages: result.scannedMessages, engine: result.engine }));
    setLocalizedHtml($('pf-vault-results'), () => (result.data || []).map(hit => '<article><h4>' + esc(hit.title) + ' · ' + esc(hit.role === 'user' ? t('ui_you') : t('ui_assistant')) + '</h4><p class="platform-message-content">' + esc(hit.snippet) + '</p><button type="button" data-vault-session="' + esc(hit.sessionId) + '" data-vault-message="' + esc(hit.messageId) + '">' + esc(t('btn_vault_open')) + '</button></article>').join('') || '<p class="platform-empty">' + esc(t('ui_vault_no_results')) + '</p>');
  }
  async function pfVaultAction(event) {
    const button = event.target.closest('button'); if (!button) return;
    if (button.dataset.vaultSession) {
      pfTab('chat'); await pfOpenSession(button.dataset.vaultSession);
      document.getElementById('pf-message-' + button.dataset.vaultMessage)?.scrollIntoView?.({ block: 'center' });
    } else if (button.dataset.vaultUse) {
      const suggestion = pfState.vault?.suggestions?.find(s => s.id === button.dataset.vaultUse); if (!suggestion) return;
      pfPrepareNewChat(); $('pf-chat-input').value = suggestion.prompt; pfTab('chat'); $('pf-chat-input').focus();
    } else if (button.dataset.vaultDismiss) {
      await pfApi('/vault/suggestions/' + encodeURIComponent(button.dataset.vaultDismiss), {}, 'DELETE'); await platformRefresh();
    }
  }
  ...
  async function pfLoadPresets() {
    ...
    setLocalizedHtml($('pf-presets-list'), () => pfState.presets.map(preset => '<article><strong>' + esc(pipelineText({...preset,isBuiltIn:true})) + '</strong><p class="muted">' + esc(pipelineText({...preset,isBuiltIn:true}, 'description')) + '</p><p>' + esc((preset.steps || []).length) + (' ' + t('ui_steps_count') + ' ') + esc((preset.steps || []).filter(step => step.requiresApproval).length) + (' ' + esc(t('ui_approval_checkpoints')) + '</p><button type="button" data-pf-install-preset="') + esc(preset.id) + '"' + (pfState.operator?.role === 'admin' ? '' : ' disabled') + ('>' + '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="12" x2="12" y1="8" y2="16"/><line x1="8" x2="16" y1="12" y2="12"/></svg>' + '<span class="action-label">' + esc(t('btn_install_preset')) + '</span>' + '</button></article>')).join('') || ('<p class="muted">' + esc(t('ui_no_coding_presets')) + '</p>'));
  }
  async function pfInstallPreset(id) {
    if (pfState.operator?.role !== 'admin') throw localizedError(() => t('error_admin_presets'));
    const body = { model: $('pf-preset-model').value, effort: $('pf-preset-effort').value || undefined, fastMode: effortFastMode('pf-preset-effort') ?? false, roleEfforts: {}, roleFastModes:{} };
    for (const role of ['planner','implementer','reviewer','security']) if ($('pf-preset-' + role + '-effort').value) body.roleEfforts[role] = $('pf-preset-' + role + '-effort').value;
    ...
    for (const role of ['planner','implementer','reviewer','security']) { const fastMode = effortFastMode('pf-preset-' + role + '-effort'); if (fastMode !== undefined) body.roleFastModes[role] = fastMode; }
  ...
  function pfEditEntry(entry) {
    ...
    if (agent) { $('pf-entry-effort').value = entry?.defaultEffort || ''; setEffortFastMode('pf-entry-effort',entry?.defaultFastMode); if (entry?.model) $('pf-entry-model').value = entry.model; $('pf-entry-mode').value = entry?.mode || 'chat'; pfHydrateSkillOptions('pf-entry-skills', (entry?.skillRefs || []).map(ref => ref.id + '@' + ref.version), () => pfState.entry === (entry || null)).catch(error => pfStatus(() => error.message, true)); }
  ...
  function pfEditProfile(profile) {
    ...
    setEffortFastMode('pf-profile-effort',profile?.defaultFastMode);
  ...
  for (const tab of ['chat','vault','memory','library','runs','system']) $('pf-tab-' + tab).addEventListener('click', () => pfTab(tab));
  $('pf-vault-search-form').addEventListener('submit', pfHandle(pfSearchVault));
  $('pf-vault-settings-form').addEventListener('submit', pfHandle(async () => { await pfApi('/vault/settings', { enabled: $('pf-vault-enabled').value === 'true', intervalMinutes: Number($('pf-vault-interval').value) }, 'PATCH'); pfState.vaultSettingsDirty = false; await platformRefresh(); }));
  for (const id of ['pf-vault-enabled','pf-vault-interval']) $(id).addEventListener('input', () => { pfState.vaultSettingsDirty = true; });
  $('pf-vault-scan').addEventListener('click', pfHandle(async () => { await pfApi('/vault/scan', {}); await platformRefresh(); }));
  for (const id of ['pf-vault-results','pf-vault-suggestions']) $(id).addEventListener('click', pfHandle(pfVaultAction));
  ...
  try { $('pf-history').open = localStorage.getItem('conduit-chat-history-open') !== 'false'; } catch {}
  $('pf-history').addEventListener('toggle', () => { try { localStorage.setItem('conduit-chat-history-open', String($('pf-history').open)); } catch {} });
  $('pf-new-project').addEventListener('click', () => pfEditProject());
  $('pf-project-cancel').addEventListener('click', () => { $('pf-project-form').hidden = true; });
  const pfProjectHandle = callback => pfHandle(async event => {
    setLocalizedText($('pf-project-status'), () => '');
    try { await callback(event); } catch (error) { setLocalizedText($('pf-project-status'), () => error.message); throw error; }
  });
  $('pf-project-form').addEventListener('submit', pfProjectHandle(pfSaveProject));
  $('pf-project-delete').addEventListener('click', pfProjectHandle(async () => {
    if (!pfState.editingProject || !confirm(t('confirm_delete_chat_project'))) return;
    await pfApi('/projects/' + encodeURIComponent(pfState.editingProject.id), {}, 'DELETE');
    $('pf-project-form').hidden = true; await platformRefresh();
  }));
  $('pf-chat-project').addEventListener('change', pfProjectHandle(async () => { try { await pfMoveChat(); } catch (error) { $('pf-chat-project').value = pfState.session?.projectId || ''; throw error; } }));
  $('pf-sessions').addEventListener('toggle', event => { const id = event.target?.dataset?.pfProject; if (id) pfState.projectOpen.set(id, event.target.open); }, true);
  $('pf-session-search').addEventListener('input', pfRenderSessions);
  $('pf-sessions').addEventListener('click', pfHandle(event => { const create = event.target.closest('[data-pf-project-new]'), edit = event.target.closest('[data-pf-project-edit]');
    if (create) { showSection('platform'); pfTab('chat'); pfPrepareNewChat(create.dataset.pfProjectNew); return; }
    if (edit) { pfEditProject(edit.dataset.pfProjectEdit); return; }
    const button = event.target.closest('[data-pf-session]'); if (button) { showSection('platform'); pfTab('chat'); return pfOpenSession(button.dataset.pfSession); } }));
  ['pf-chat-profile','pf-chat-agent','pf-run-agent'].forEach(id => $(id).addEventListener('change',() => {
    const run = id === 'pf-run-agent', prefix = run ? 'pf-run' : 'pf-chat';
    const agent = pfState.agents.find(item => item.id === $(prefix + '-agent').value);
    const profile = !run && pfState.profiles.find(item => item.id === $('pf-chat-profile').value);
    setEffortFastMode(prefix + '-effort',agent?.defaultFastMode ?? profile?.defaultFastMode ?? false);
    syncEffortControls();
  }));
  ...
  $('pf-evaluation-form').addEventListener('submit', pfHandle(async () => { const selected = pfValues('pf-evaluation-models'); if (!selected.length || selected.length > 8) throw localizedError(() => t('error_evaluation_models')); await pfApi('/evaluations', { models: selected, effort: $('pf-evaluation-effort').value || undefined, fastMode: effortFastMode('pf-evaluation-effort') ?? false }); await platformRefresh(); pfStatus(() => t('ui_evaluations_queued')); }));
  ...
  setInterval(() => {
    if (activeSection === 'platform' && !document.hidden && (pfState.tab === 'vault' || (pfState.tab === 'runs' && pfState.runs.some(run => ['queued','running'].includes(run.status))))) platformRefresh().catch(error => pfStatus(() => error.message, true));
  }, 3000);
`;
