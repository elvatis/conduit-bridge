...
export const EFFORT_SCRIPT = String.raw`
  ...
  function positionEffortPopover(control) {
    ...
    const width = card.getBoundingClientRect().width || Math.min(310,window.innerWidth - 24), height = card.offsetHeight || 173;
  ...
  document.addEventListener('scroll',event => {
    ...
    document.querySelectorAll('.effort-trigger[aria-expanded="true"]').forEach(trigger => {
      const control = trigger.closest('.effort-control'), rect = trigger.getBoundingClientRect();
      if (rect.bottom < 0 || rect.top > window.innerHeight) closeEffortPopover(control); else positionEffortPopover(control);
    });
  },true);
