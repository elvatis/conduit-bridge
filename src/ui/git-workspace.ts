...
(() => {
  ...
  function renderSidebar() {
    const s=state.snapshot; $g('gw-branch').textContent=s?.branch || (s?.head ? s.head.slice(0,7)+' ('+copy('detached')+')' : '-'); $g('gw-branch').title=$g('gw-branch').textContent; $g('gw-head').textContent=(s?.head||'').slice(0,7); $g('gw-head').title=s?.head||''; $g('gw-root').textContent=s?.root||''; $g('gw-root').title=s?.root||'';
  ...
  function renderLog() {
    ...
    $g('gw-filter-branch').title=$g('gw-filter-branch').textContent;
    ...
    else { const width=Math.max(30,...s.commits.map(commit=>commit.graph.width*12+9)); $g('gw-log').innerHTML=s.commits.length?s.commits.map((commit,index)=>'<button type="button" class="gw-commit" data-gw-commit="'+escape(commit.sha)+'" aria-pressed="'+(state.selected===commit.sha)+'" title="'+escape(commit.sha+' · '+commit.subject)+'">'+graph(commit,index,width)+'<span class="gw-commit-copy">'+(commit.refs.length?'<span class="gw-refs">'+commit.refs.map(ref=>'<span class="gw-ref'+(ref.includes('/')?' remote':'')+'" title="'+escape(ref)+'">'+escape(ref)+'</span>').join('')+'</span>':'')+'<span class="gw-commit-subject" title="'+escape(commit.subject)+'">'+escape(commit.subject)+'</span></span><span class="gw-author" title="'+escape(commit.author)+'">'+escape(commit.author)+'</span><time class="gw-age" datetime="'+escape(commit.date)+'">'+escape(age(commit.date))+'</time></button>').join(''):empty(copy('emptyHistory')); }
