...
<section id="git-workspace" class="gw-page" aria-label="Git workspace">
  ...
  <div class="gw-shell">
    <aside class="gw-sidebar" aria-label="Repository navigation">
      <div class="gw-workspace-choice"><label class="gw-label" for="gw-workspace" data-gw-copy="workspace">WORKSPACE</label><select id="gw-workspace" aria-label="Workspace"></select></div>
      <div class="gw-repo-meta"><span class="gw-branch" id="gw-branch">—</span><span id="gw-head" class="gw-mono"></span></div>
      <nav class="gw-views" aria-label="Repository views"><button type="button" data-gw-view="changes"><span>±</span><span data-gw-copy="changes">Changes</span><b id="gw-change-count">0</b></button><button type="button" data-gw-view="history" aria-pressed="true"><span>⑂</span><span data-gw-copy="history">History</span></button></nav>
    ...
    <main class="gw-history">
      ...
      <div id="gw-log" class="gw-log" aria-label="Commit history"></div><footer class="gw-history-foot"><span id="gw-history-note"></span><span class="gw-mono">GIT</span></footer>
    </main>
    <aside class="gw-inspector" aria-label="Change inspector"><div id="gw-inspector-meta" class="gw-inspector-meta"></div><div id="gw-files" class="gw-files" aria-label="Changed files"></div><div id="gw-diff-banner" class="gw-diff-banner"></div><div id="gw-diff" class="gw-diff" tabindex="0" aria-label="Unified diff"></div></aside>
  </div>
  <dialog id="gw-action-dialog" class="gw-dialog"><form id="gw-action-form"><h3 id="gw-action-title"></h3><p id="gw-action-description"></p><label for="gw-action-name" data-gw-copy="branchName">Branch name</label><input id="gw-action-name" name="name" maxlength="120" autocomplete="off" placeholder="codex/feature-name" required><div class="gw-dialog-actions"><button type="button" id="gw-action-cancel" data-gw-copy="cancel">Cancel</button><button type="submit" class="gw-primary"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" aria-hidden="true"><path d="M12 5v14M5 12h14"/></svg><span class="action-label" data-gw-copy="create">Create</span></button></div></form></dialog>
</section>`;
...
(() => {
  ...
  function applyCopy() { const root = $g('git-workspace'); if (!root) return; root.querySelectorAll('[data-gw-copy]').forEach(node => node.textContent = copy(node.dataset.gwCopy)); root.querySelectorAll('[data-gw-title]').forEach(node => { node.title=copy(node.dataset.gwTitle); node.setAttribute('aria-label',node.title); }); }
  ...
  function age(date) { const minutes=Math.max(0,Math.floor((Date.now()-new Date(date).getTime())/60000)); return minutes<1 ? copy('now') : minutes<60 ? minutes+'m' : minutes<1440 ? Math.floor(minutes/60)+'h' : Math.floor(minutes/1440)+'d'; }
