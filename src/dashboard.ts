...
Events:   ws://127.0.0.1:31338/v1/events</pre><p>Use <code>fallback_models</code> for ordered recovery and <code>effort</code> or <code>reasoning_effort</code> where supported.</p></section>
    <section><h3>Security and troubleshooting</h3><ul><li>Keep 31338 on the local desktop loopback interface.</li><li>Activity and metrics never store prompts, responses, cookies, or credentials.</li><li>If a CLI is unavailable, verify that it is installed, authenticated, and on PATH.</li><li>API keys and CLI authentication are deliberately independent; configure the transport you intend to use.</li></ul><p>The standalone <a class="link" href="/help">Help page</a>, <code>docs/AUTOSTART.md</code> contain the full deployment guide.</p></section>
  </section>
</div></main><script>
  ...
  const stateText = p => p.connected
    ...
    : (p.loginType === 'api-key' ? 'No API credential · add a key in Settings' : p.loginType === 'cli' ? 'CLI unavailable · install or authenticate the tool' : 'Local service unavailable');
...
WS   /v1/events</pre><p>The base URL for clients is <code>http://127.0.0.1:31338/v1</code>. Chat supports SSE streaming, <code>fallback_models</code>, and provider-specific <code>effort</code> or <code>reasoning_effort</code>.</p></section>
  ...
  <section><h2>Security</h2><ul><li>Loopback is the required desktop bind.</li><li>Cross-site state-changing requests and foreign WebSocket origins are rejected.</li><li>Activity and metrics do not store prompts, responses, cookies, or credentials.</li></ul></section>
