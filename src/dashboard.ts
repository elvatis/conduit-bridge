...
Request:  POST /chat/completions</pre><div class="play-actions"><button id="test-all-cli" type="button">Test all CLI providers</button><button id="test-openai" type="button">Test OpenAI-compatible route</button></div><div id="test-output" class="activity-list">No integration test run yet.</div></section>
  ...
  <section id="help-section-v2" class="wide page-section"><h2>Help and operating guide</h2><p class="muted">Install, connect clients, sign in to browser providers, and diagnose the bridge.</p>
    <section><h3>Supported desktop platforms</h3><p>Conduit Bridge currently supports Windows Desktop and Linux Desktop. It needs Node.js 24 or newer and Chromium. OpenClaw is optional. Start the bridge on the desktop and open <code>http://127.0.0.1:31338/</code>.</p></section>
...
Events:   ws://127.0.0.1:31338/v1/events</pre><p>Use <code>fallback_models</code> for ordered recovery and <code>effort</code> or <code>reasoning_effort</code> where supported.</p></section>
    <section><h3>Security and troubleshooting</h3><ul><li>Keep 31338 on loopback and use SSH forwarding for remote access.</li><li>An external bind requires <code>authToken</code> and an explicit origin allowlist.</li><li>Activity and metrics never store prompts, responses, cookies, or credentials.</li><li>If login cannot start, verify Chromium and that Conduit is running inside the logged-in desktop session.</li><li>If restore fails after login, inspect Technical details and try the matching <code>api-*</code> or <code>cli-*</code> transport.</li></ul><p>The standalone <a class="link" href="/help">Help page</a>, <code>docs/AUTOSTART.md</code>, and <code>docs/BROWSER-LOGIN.md</code> contain the full deployment guide.</p></section>
  </section>
</div></main><script>
  ...
  function loginCardHtml(provider) {
    ...
    const rows = [
      ...
      ['Window manager', env.windowManager === false ? 'Not detected' : 'Present'],
    ];
...
<body><main class="help"><header><div><h1>Conduit Bridge Help</h1><div class="muted">Installation, browser login, provider routing, and operations</div></div><nav class="nav"><a class="link" href="/">Dashboard</a></nav></header>
  <section><h2>Requirements and installation</h2><p>Conduit Bridge runs independently of OpenClaw. The current supported platforms are Windows Desktop and Linux Desktop. It requires Node.js 24 or newer and Chromium.</p><pre>npm install
...
node dist/cli.js status</pre><ul><li><strong>No graphical session:</strong> run the bridge inside the logged-in Windows or Linux desktop session.</li><li><strong>Profile already in use:</strong> stop the active browser. Conduit clears only stale locks.</li><li><strong>Security check does not resolve:</strong> use the matching <code>api-*</code> or <code>cli-*</code> transport.</li><li><strong>Restore is signed out:</strong> start a fresh login and inspect Technical details on the provider card.</li><li><strong>Remote dashboard unavailable:</strong> verify the SSH tunnel and that 31338 listens on server loopback.</li></ul><p>See <code>docs/AUTOSTART.md</code> and <code>docs/BROWSER-LOGIN.md</code> for the deployment guide.</p></section>
</main></body></html>`;
